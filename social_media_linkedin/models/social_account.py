...
class SocialAccount(models.Model):
        ...
        for organization_id in organization_ids:
            ...
            if isinstance(response_organizations, dict):
                ...
                if (
                    ...
                    complete_url = list(
                        filter(
                            lambda x: "logo_400_400" in x.get("artifact", ""), elements
                        )
                    )
                    if not complete_url and elements:
                        complete_url = elements[0]
                    identifiers = (
    ...
    def validate_access_token(self):
        ...
        if (
            self.media_id.id
            == self.env.ref("social_media_linkedin.social_media_linkedin").id
        ):
