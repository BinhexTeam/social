...
class ResConfigSettings(models.TransientModel):
    ...
    x_use_account = fields.Boolean(
        string="Account x",
        config_parameter="connector_social_x.x_use_account",
    )
    x_api_key = fields.Char(
        string="API Key",
        config_parameter="connector_social_x.x_api_key",
    )
        ...
        for record in self:
            if self.env.user.has_group("base.group_system"):
                record.x_api_secret = (
                    ...
                    .sudo()
                    .get_param("connector_social_x.x_api_secret")
                )
            else:
        ...
        for record in self:
            if self.env.user.has_group("base.group_system"):
                self.env["ir.config_parameter"].sudo().set_param(
                    "connector_social_x.x_api_secret", record.x_api_secret
                )
